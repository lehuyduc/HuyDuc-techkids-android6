...
public class Bot {
    ...
    protected void copy(Bot other) {
        ...
        for (int i=0;i<nPoints;i++) {points[i] = other.points[i]; points2[i] = other.points2[i];}
    }
    ...
    //**********  DESTRUCTOR  ***************************************************************
    public boolean oos() {
        return (x < -sizeX/2 || x>=BACKGROUND_WIDTH+sizeX/2 || y<=-sizeY/2 || y>=BACKGROUND_HEIGHT+sizeY/2);
    }

    public boolean needDestruct() {
        return (oos() || dead);
    }

    public void destruct() {
        setPhoto("resources/transparent.png");
        setSizeX(0); setSizeY(0);
        setX(-10); setY(-10);
    }
