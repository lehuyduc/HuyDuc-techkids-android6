 ...
 */
public class EnemyPlaneController extends SingleController implements Colliable, Subscriber {
    ...
    void checkView() {
        ...
        if (enemyPlaneType==EnemyPlaneType.YELLOW) {
            gameView = new AnimationView("enemy_plane_yellow.png",3,32,32, GameConfig.animationDelay);
        }
    ...
    @Override
    public GameObject getSubscriberObject() {
        return gameObject;
    }
    ...
    public void onEvent(EventType eventType, SingleController sender) {
        if (eventType==EventType.BOMB_EXPLODE) {
            Bomb bomb = (Bomb)(sender.getGameObject());
            if (Utils.distance(gameObject,bomb) <= bomb.getDamageRadius())
                gameObject.takeDamage(bomb.getDamage());
        }
    }
